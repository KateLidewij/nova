...
<div id="manifest" class="hide">
	<?php if (isset($manifests)): ?>
		<div>
			<strong><?php echo $label['manifests'];?></strong> &mdash;
			<?php $i = 1;?>
	...
	<div>
		<strong><?php echo $label['toggle'];?></strong> &mdash;
		<?php if($display == 'open'): ?>
		<?php else: ?>
	...
	<?php if (isset($depts)): ?>
		...
		<?php foreach ($depts as $dept): ?>
			<tr>
				<td colspan="5"><h4><?php echo $dept['name'];?></h4></td>
			</tr>
				...
				<?php foreach ($dept['pos'] as $pos): ?>
					...
					<?php if (isset($pos['chars'])): ?>
						<?php foreach ($pos['chars'] as $char): ?>
							...
							<tr class="fontSmall hide <?php echo $char['crew_type'] . $display;?>">
								<td class="col_15"></td>
								...
								<td>
									...
									<?php if ($char['crew_type'] == 'npc'): ?>
										<br><?php echo text_output($label['npc'], 'span', 'muted');?>
									<?php elseif ($char['crew_type'] == 'inactive'): ?>
										<br><?php echo text_output($label['inactive'], 'span', 'muted');?>
									<?php endif; ?>
								</td>
								<td></td>
					...
					<?php if ($pos['open'] > 0 and $dept['type'] == 'playing'): ?>
						<tr class="open fontSmall hide">
							<td class="col_15"></td>
			...
			<?php if (isset($dept['sub'])): ?>
				<?php foreach ($dept['sub'] as $sub): ?>
					<tr>
						<td class="col_15"></td>
						<td colspan="4"><h5><?php echo $sub['name'];?></h5></td>
					</tr>
